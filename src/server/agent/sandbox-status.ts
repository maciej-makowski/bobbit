...
export async function checkDockerAvailability(
	...
	try {
		...
		// Append the runtime's actionable hint (e.g. Podman rootless/SELinux/`info`
		// schema gotchas) so the UI doesn't just say "not available".
		const hint = runtime.availabilityHint();
		return { available: false, runtime: runtime.id, error: hint ? `${String(err)} — ${hint}` : String(err) };
	}
}
