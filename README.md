...
### Marketplace
Distribute roles, tools, and skills as self-contained **packs** — directories that can also contribute panels, entrypoints, and routes. **User packs** come from a git repo or local source: installing copies the pack in, uninstalling deletes it. Bobbit also ships **built-in first-party packs** that are auto-resolved in place — active by default, toggle-only (you enable/disable, never install or uninstall), and non-removable as a source — so shipped features (e.g. the PR-walkthrough viewer) flow through the same pack API. Resolution walks one ordered list of packs so entities show up on the Roles/Tools/Skills pages tagged with their origin pack. See [docs/marketplace.md](docs/marketplace.md) and its [built-in packs section](docs/marketplace.md#built-in-first-party-packs).
