/**
 * Integration tests for the multi-gateway provider surface.
 *
 * Ported from the fork's tests/e2e/multi-gateway-api.spec.ts onto the tests2
 * in-process integration harness (see tests2/integration/aigw-api.test.ts and
 * aigw-configure.test.ts for the setup idioms this mirrors).
 *
 * Exercises the REST contract against TWO in-process stub gateways (NO NETWORK):
 *
 ...
 *     `claude-local` stays openai-completions.
 ...
 *   - per-gateway refresh + status by name; validation 400s.
 ...
 * reachable from the in-process REST harness.
 */
...
// Tests share the fork-scoped gateway + a single on-disk models.json, so they
...
test.describe("Multi-gateway provider API", () => {
	...
	test("merged mode: enabled openai-compatible + disabled aigw", async ({ gateway }) => {
		...
		const getRes = await apiFetch("/api/aigw/gateways");
		expect(getRes.status).toBe(200);
		const getData = await getRes.json();
		expect(getData.gateways).toHaveLength(2);
		expect(getData.gateways.map((g: any) => g.name).sort()).toEqual(["aigw", "llama-swap"]);
	...
	test("PUT rejects invalid gateway lists (400): bad name, aigw-not-named-aigw, dup", async () => {
		// An aigw-type gateway must be named exactly "aigw" (singleton constraint).
		const notAigw = await putGateways([
			{ name: "enterprise", type: "aigw", url: stubB.url, enabled: true },
		]);
		expect(notAigw.status).toBe(400);
		expect((await notAigw.json()).error).toBeTruthy();

		// Name must match ^[a-zA-Z0-9._-]+$.
		const badName = await putGateways([
			{ name: "bad name!", type: "openai-compatible", url: stubA.url, enabled: true },
		]);
		expect(badName.status).toBe(400);
		expect((await badName.json()).error).toBeTruthy();

		// Duplicate names are rejected.
		const dup = await putGateways([
			{ name: "dup", type: "openai-compatible", url: stubA.url, enabled: true },
			{ name: "dup", type: "openai-compatible", url: stubB.url, enabled: true },
		]);
		expect(dup.status).toBe(400);
		expect((await dup.json()).error).toBeTruthy();
	});
