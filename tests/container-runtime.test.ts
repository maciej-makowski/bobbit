...
describe("availabilityHint()", () => {
	it("DockerRuntime returns undefined (no runtime-specific gotchas)", () => {
		assert.equal(new DockerRuntime().availabilityHint(), undefined);
	});

	it("PodmanRuntime returns a non-empty actionable hint", () => {
		const hint = new PodmanRuntime().availabilityHint();
		assert.equal(typeof hint, "string");
		assert.ok((hint as string).length > 0);
		// Names the common rootless/SELinux + info-schema gotchas.
		assert.match(hint as string, /rootless/i);
		assert.match(hint as string, /SELinux/i);
		assert.match(hint as string, /\.Version\.Version/);
	});
});
