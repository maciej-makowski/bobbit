...
## Addendum — interactive setup command + runtime selection (follow-up)

This addendum records follow-up changes merged after the original design above. Sections 1–9 still describe the chosen architecture and rationale; the points below refine the *operator entry point* and the *Node runtime* without changing that architecture.

- **Interactive entry point — `npm run setup-cloudflare-tunnel`.** A new host-only installer, `deploy/cloudflare/setup-tunnel.sh` (wired to the `setup-cloudflare-tunnel` npm script), is now the recommended way to deploy. It performs host dependency checks (`podman`, `systemctl --user`, `loginctl` required; `cloudflared` warn-only), prompts for `CF_HOSTNAME` / `TUNNEL_NAME` / `PORT` / `CHECKOUT_DIR` / `BOBBIT_CWD` / `RUNTIME` / `TOOLBOX_CONTAINER` / `NODE_BIN_DIR` (each pre-seedable via the matching env var, with `--yes`/`-y`/`ASSUME_DEFAULTS=1` and `--help`/`-h`), warns on a 2nd-level subdomain, scaffolds `~/.config/cloudflared/config.yml` (hostname + port substituted, `<TUNNEL_ID>` left as a placeholder, prompts before overwrite), installs the units by delegating to `setup.sh` (kept as the single source of truth for unit rendering), prints the remaining manual Cloudflare-panel steps, and offers a strictly opt-in (default No) run of `cloudflared tunnel login/create/route dns` when the CLI is present. It never runs the Access-dashboard step.
- **`RUNTIME=host|toolbox` selectable option (host is the default).** §4's analysis rejected the toolbox Node *for `ExecStart`* and chose host Node as the default — that still holds. The follow-up makes the toolbox runtime an explicit *opt-in*: with `RUNTIME=toolbox` + `TOOLBOX_CONTAINER`, `setup.sh` validates Node ≥ 20 inside the container and wraps `ExecStart` in a `toolbox run -c <container> ` prefix. The boot-time fragility called out in §4 (toolbox/Podman lifecycle + `XDG_RUNTIME_DIR` coupling) is exactly why `RUNTIME=host` remains the default and recommended choice; the toolbox runtime exists for hosts that keep Node only in a toolbox.
- **Unit parametrization — `<PORT>` and `<EXEC_PREFIX>`.** `bobbit.service` gained two placeholders beyond the original `<CHECKOUT_DIR>` / `<BOBBIT_CWD>` / `<NODE_BIN_DIR>`: `<PORT>` (the loopback port, default `3001`) and `<EXEC_PREFIX>` (empty for the host runtime; `toolbox run -c <container> ` for the toolbox runtime). `ExecStart` is now `<EXEC_PREFIX><CHECKOUT_DIR>/run --host 127.0.0.1 --no-tls --port <PORT> --cwd <BOBBIT_CWD>`. `setup.sh` accepts the corresponding `PORT` and `EXEC_PREFIX` / `RUNTIME` / `TOOLBOX_CONTAINER` env vars.
- **Default hostname is now the first-level `bobbit-z13.maciej.dev`.** §3's decision matrix flagged option A (first-level name, free Universal SSL) as the recommended fallback. The follow-up promotes it to the *default*: `config.yml.example` and the interactive installer now default to `bobbit-z13.maciej.dev`, so the zero-cost TLS path works out of the box. The 2nd-level `bobbit.z13.maciej.dev` remains documented as the "if you must, here is the paid cost" case (Total TLS / Advanced Certificate). The `<HOSTNAME>` placeholder convention in the runbook is unchanged.

## Addendum 2 — mount cloudflared's default `~/.cloudflared` (follow-up)

The original design (§2, §5.2) mounted `~/.config/cloudflared` into the quadlet and required the operator to **copy** the credentials JSON there (and the runbook's step 5 did so). In practice that copy step was easy to miss and produced a confusing failure: cloudflared could not find its credentials, fell back to looking for the origin `cert.pem`, and crashed with `error parsing tunnel ID: ... client didn't specify origincert path`. This was hit live during first deployment.

**Change:** the quadlet now mounts cloudflared's **default** directory `~/.cloudflared` directly — the dir where `cloudflared tunnel login` already writes `cert.pem` and `cloudflared tunnel create` already writes `<UUID>.json`. There is nothing to copy, and `config.yml` lives at `~/.cloudflared/config.yml` (also the host CLI's default config path).

**Two gotchas this surfaced, and how they're resolved:**
- **Nonroot image + rootless = can't read owner-only files.** The official `cloudflare/cloudflared` image is built `FROM gcr.io/distroless/base-debian13:nonroot`, so the daemon runs as uid 65532. Under rootless Podman that maps to a subuid (not the host user), so it cannot traverse `~/.cloudflared` (mode `0700`) or read `cert.pem` (`0600`) / `<UUID>.json` (`0400`). **Resolution:** the quadlet sets **`User=0`**; in rootless Podman container-root maps to the host user (the file owner), so the default perms are readable with **no chmod**. (cloudflared then logs a harmless `ping_group_range` GID warning — it just can't ICMP, which the HTTP/WS tunnel never needs.)
- **Shared SELinux relabel.** Because `~/.cloudflared` is shared between the container and the host `cloudflared` CLI, the mount uses the **shared** relabel `:z` (not the private `:Z` used originally for the container-only `~/.config/cloudflared`).

Net effect: `cloudflared tunnel login` + `create` + setting `<TUNNEL_ID>` in `~/.cloudflared/config.yml` is sufficient — no credential copying and no permission tweaking. `setup.sh` / `setup-tunnel.sh` set `CLOUDFLARED_DIR=$HOME/.cloudflared` accordingly and dropped the copy step from their printed manual instructions. This supersedes the `~/.config/cloudflared` + `:ro,Z` + copy-creds approach in §2/§5.2.
