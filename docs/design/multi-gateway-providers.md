...
Status: design record (ported from the fork's `master-old`, adapted to
`G-Research/bobbit` master). Tracked in goal `goal/multi-gateway-8e973d9d`.

> **Porting note.** This design was authored on the fork before upstream gained
> well-known-first discovery and the cross-origin DNS-rebinding guard. On
> `G-Research/bobbit` master, all of that hardening already exists for the single
> `aigw` provider; multi-gateway **keeps it** and moves it onto the `aigw`-type
> gateway inside the `modelGateways` list. Where this record describes the `aigw`
> writer / discovery as "byte-for-byte today's behavior", read that as *today's
> upstream behavior including well-known routing, remote-config admission, the
> DNS guard, and default-model seeding* — none of which is removed. See
> [docs/ai-gateway-routing.md](../ai-gateway-routing.md) and
> [docs/internals.md](../internals.md#ai-gateway-request-headers-user-agent-x-opencode-session).
...
named, typed gateways.** No new daemon. Image generation is explicitly out of
scope.

> **Custom-provider path (fork PR #6).** The manual custom-provider metadata
> path (`model-registry.ts::discoverCustomProviderModels` / `mapManualModels`)
> is **not** used, reused, or repaired by this feature. Fork PR #6 (which added
> that path) was **never ported to `G-Research/bobbit` master**, so there is
> nothing to revert here — the custom-provider path simply plays no role.
> Local providers are served by an `openai-compatible` **gateway**, which is the
> path that actually binds via `set_model`.
...
| **`aigw` type** | Enterprise AI-Gateway: well-known-first discovery, Bedrock-routes Claude ids, sends special headers, DNS-rebinding guard, is **exclusive**. |
| **`openai-compatible` type** | Plain OpenAI gateway (ollama/llama-swap/vLLM/…): `GET /v1/models` discovery, no Bedrock, no special headers, no DNS guard, never exclusive. |
...
`default.namingModel`, role/review prefs) continue to resolve unchanged — and
the well-known / DNS-guard hardening keyed on that provider is preserved.
...
const DISCOVERY: Record<GatewayType, (url: string) => Promise<AigwModel[]>> = {
	"aigw": discoverAigwModels,            // well-known-first, with /v1/models fallback + DNS guard
	"openai-compatible": discoverOpenAiCompatibleModels, // plain GET /v1/models (see §5)
};
```

> The `aigw` discovery function is upstream's existing `discoverAigwModels()` —
> well-known-first (`/.well-known/opencode`) with the legacy `/v1/models` +
> `inferMeta` fallback, remote-config admission, and the cross-origin
> DNS-rebinding guard, all unchanged. The `openai-compatible` discovery is the
> plain `GET /v1/models` path only, with none of that machinery.

### 3a. `aigw` writer — `buildAigwProviderBlock(gateway, models)`

**Behaves exactly like today's `writeAigwModelsJson`** (well-known-driven
routing/metadata, Bedrock-routed Claude models, generated headers), just keyed
by `gateway.name` instead of the literal `aigw` and reading the URL from
`gateway.url`:

- `baseUrl` = `gateway.url` with trailing slashes stripped.
- `apiKey: "none"`, `api: "openai-completions"` / `openai-responses` per
  well-known routing.
...
- Non-Claude ids → `openai-completions` / `openai-responses` (per well-known)
  with the conservative `openaiCompat` flags merged with any `m.compat`.
...
- **Never** `bedrock-converse-stream`, **never** a per-model `/aws` baseUrl,
  **never** Bedrock env, **never** well-known discovery or a DNS guard.
...
**Metadata (incl. vision labeling) is deferred to `inferMeta`** for an
`openai-compatible` gateway — a bare local VLM id may not be *labeled*
vision-capable (no `image` in `input`) until the future `ollama` native type
lands (§9). Document this limitation; do not build metadata overrides. (The
`aigw` type takes authoritative per-model metadata from its well-known config.)
...
- **Predicate** in `getFilteredModels()`:
  ...
  always emitted with `authenticated:true` (§4, `model-registry.ts`), and
  custom-provider rows are likewise emitted `authenticated:true`, so only
  built-in cloud models can ever be `authenticated:false`. Document this
