...
- **Unrelated failure that surfaced once the suite started exiting**: with the hang fixed and the suite running to completion, the pre-existing `proposal-spec-survives-navigate` E2E reproducer began failing deterministically. It was re-quarantined (`test.fixme`) with explicit user permission and is tracked in [docs/design/proposal-spec-rehydrate.md](design/proposal-spec-rehydrate.md) plus a dedicated follow-up goal — it is not part of this fix and the quarantine was left untouched.
