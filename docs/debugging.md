...
- **Status row shows the wrong runtime / "podman selected but it's probing docker"?** Fixed: `GET /api/sandbox-status` takes an optional `?sandbox=<none|docker|podman>` param and the Settings UI passes the **pending dropdown selection** (`fetchSandboxStatus(mode)`), so the Runtime Status row probes the backend you just picked rather than the saved `project.yaml` runtime — it updates the instant you change the dropdown, before saving. If the param is omitted the endpoint falls back to the saved-config runtime (`resolveContainerRuntime`). If the row looks stale, confirm the `@change` handler re-fired `loadSandboxStatus(value)` (it resets `sandboxStatusLoaded`).
- **Building the image**: `npm run sandbox:build:docker` / `npm run sandbox:build:podman` build + tag `bobbit-agent` from `docker/Dockerfile` with the respective runtime, baking the host's `pi-coding-agent` version (`--build-arg PI_AGENT_VERSION=<host>`). Override the tag with `SANDBOX_IMAGE=<tag>`. Docker mode auto-builds on first use; under Podman, build the image yourself (rootless/SELinux setup is host-specific) — use the npm script or the Settings → Container Sandbox **Build Image** button. Restart the server after a manual build so the pool reloads.
...
- **Agent dies under rootless Podman with `EACCES … mkdir '/home/node/.bobbit/agent/sessions/…'`** (container ready, clone + worktree OK, agent process launched then exits): the agent's `getDefaultSessionDir()` can't write the HOST bind mount because, without `--userns=keep-id`, the container's `node` (uid 1000) maps to a host SUBUID that does not own the host-owned dir. Fix: `PodmanRuntime` emits `--userns=keep-id:uid=1000,gid=1000` (first flag after `run -d`), mapping the host user to container uid 1000 → host bind mounts are node-writable, no chown. Affects every writable host bind mount (`/home/node/.bobbit/agent/sessions`, `/bobbit-state/*`, `/bobbit/preview*`, `/tmp/session-prompts`). Note keep-id is necessary *and* the `:Z` relabel is also required on rootless/SELinux hosts (the host SELinux may enforce even when a toolbox `getenforce` reports Disabled). Stale containers created before keep-id are auto-recreated by `ProjectSandbox._initContainer()`'s bind-mount write-probe. Validate on real podman with `CONTAINER_HOST=unix:///run/user/$(id -u)/podman/podman.sock npx tsx scripts/podman-e2e-validate.ts` (Scenarios D + E).
