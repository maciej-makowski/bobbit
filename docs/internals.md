...
- `DockerRuntime` and `PodmanRuntime` each `extend BaseCliRuntime` and override only what genuinely differs. The deltas are confined to two areas:
  ...
  - **`run` args.** *User namespace (rootless bind-mount writability):* Podman emits `--userns=keep-id:uid=1000,gid=1000` as the very first flag after `run -d` (via `RunArgHooks.extraRunArgs()`); Docker emits nothing there. Without keep-id the container's `node` (uid 1000) maps to a host SUBUID that does not own the writable HOST bind mounts (`/home/node/.bobbit/agent/sessions`, `/bobbit-state/*`, `/bobbit/preview*`, `/tmp/session-prompts`), so the agent dies on `EACCES … mkdir '/home/node/.bobbit/agent/sessions/…'`. keep-id maps the *host* user to container uid/gid 1000 so those mounts are read/write by `node` AND the host gateway still owns/reads them — no host chown, no `:U`. (keep-id alone is necessary but not sufficient: a rootless/SELinux host *also* needs the `:Z` relabel below.) *Host-gateway:* Docker emits `--add-host=host.docker.internal:host-gateway`; Podman provides `host.containers.internal` natively and *also* maps the Docker name, so agent tooling that hardcodes `host.docker.internal` keeps working. *SELinux:* Podman appends `:Z` (private relabel) to relabel-eligible bind mounts so a rootless/SELinux host can write into them; Docker ignores relabel flags entirely. *Network:* Docker disables inter-container comms with `--opt com.docker.network.bridge.enable_icc=false`, an opt Podman rejects, so it is omitted. A shared serializer (`serializeContainerRunSpec`) fixes the run-arg skeleton order and delegates only these differences to per-runtime `RunArgHooks`; `buildDockerRunArgs` (in `docker-args.ts`) is the Docker serialization of the same spec and its byte-for-byte output is pinned by a test so Docker behaviour never drifts. Containers created before keep-id are auto-healed: `ProjectSandbox._initContainer()` write-probes a bind mount as `node` on every reconnect/restart and recreates the container (named volumes preserve worktrees) if the probe EACCEes.
