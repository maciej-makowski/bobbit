...
**Per-gateway scope (multi-gateway).** Gateways are now a list persisted under the `modelGateways` preference, and each has a `type`. Everything in this section — well-known-first discovery, the cross-origin DNS-rebinding guard, per-model well-known metadata/cost, default-model seeding, Bedrock-routed Claude models, and the generated `providers.<name>.headers` block — applies to the **`aigw`-type** gateway only. That gateway is a pinned singleton still named exactly `"aigw"`, so every `"aigw"`-keyed path below is unchanged; the description reads per-gateway rather than for a single hardcoded endpoint. The other type, **`openai-compatible`** (ollama, llama-swap, vLLM, …), is served as plain OpenAI — `GET /v1/models` discovery only, **no** well-known/remote-config, **no** DNS guard, **no** `x-opencode-session`/User-Agent header block, and **never** Bedrock — so a model literally named `claude-*` behind such a gateway stays on `openai-completions`. See [docs/multi-gateway-providers.md](multi-gateway-providers.md) for the gateway list model, types, and derived exclusivity (any enabled `aigw` gateway hides built-ins and every `openai-compatible` gateway).
