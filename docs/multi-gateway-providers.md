...
> For the enterprise `aigw`-type gateway's well-known discovery, provider
> routing, remote-config security, and DNS-rebinding guard, see
...
multiple gateways need — well-known-first discovery, `models.json` writes,
browser proxying, startup re-discovery, title-gen routing, and the cross-origin
DNS-rebinding guard — it was just one instance and carried a fragile global
heuristic ("model id contains `claude` ⇒ route through Bedrock").
...
| **Discovery** | **Well-known-first** (`/.well-known/opencode`), with `/v1/models` + `inferMeta` fallback; cross-origin provider DNS-rebinding guard (see [AI Gateway routing](ai-gateway-routing.md)) | `GET /v1/models` → `inferMeta` only — no well-known, no remote-config, no DNS guard |
...
| **`models.json` `api`** | `openai-completions` / `openai-responses` (+ `bedrock-converse-stream` for Claude ids) | `openai-completions` only |
...
The `aigw` type **retains every upstream AI-Gateway hardening** unchanged —
well-known-first discovery and provider routing, remote-config admission, the
cross-origin DNS-rebinding guard, per-model cost/metadata, default-model
seeding, and the generated `providers.aigw.headers` block. Multi-gateway only
moves that behavior from a single hardcoded `"aigw"` key onto the `aigw`-type
gateway inside the `modelGateways` list. `openai-compatible` gateways
deliberately receive **none** of it. See
[docs/internals.md — AI Gateway request headers](internals.md#ai-gateway-request-headers-user-agent-x-opencode-session)
and [AI Gateway routing](ai-gateway-routing.md).
...
See [docs/rest-api.md — AI Gateway](rest-api.md#ai-gateway) for the full request
/ response shapes.
...
`default.namingModel`, role / review model prefs) keep resolving unchanged — and
all well-known/DNS-guard behavior carries over untouched.
...
1. Discovers each **enabled** gateway and writes its `providers.<name>` block via
   ...
   `buildOpenAiCompatibleProviderBlock`). The `aigw` writer preserves the full
   upstream behavior (well-known cost/metadata, Bedrock-routed Claude models,
   `providers.<name>.headers`); the `openai-compatible` writer emits plain
   OpenAI blocks with no headers.
...
`inferMeta` for an `openai-compatible` gateway. A bare local VLM id may not be
*labeled* vision-capable (no `image` in its `input`) in the picker until a
future native `ollama` type lands — even though images still pass through. This
is a labeling gap, not a functional one. (The `aigw` type instead takes
per-model metadata from its well-known config, so its labeling is authoritative.)
...
- **Native `ollama` / `llama-server` discovery types** — only the seam exists
  today; discovery is well-known-first for `aigw` and `GET /v1/models` for
  `openai-compatible`.
- **Accurate per-model metadata / manual overrides for bare `openai-compatible`
  ids** — deferred to the future native types (which expose real capability
  metadata).
- **Image generation** — not addressed by this feature.
- **The custom-provider path** (`model-registry.ts` manual models) — explicitly
  not reused or repaired. That manual custom-provider metadata path (fork PR #6)
  was **never ported to `G-Research/bobbit` master**; local providers are
  supported via an `openai-compatible` **gateway** instead, which is the path
  that actually binds via `set_model`.
...
- [docs/ai-gateway-routing.md](ai-gateway-routing.md) — the enterprise
  `aigw`-type gateway's well-known discovery, provider routing, remote-config
  security, model-ID migration, and DNS-rebinding / cache / container behavior.
- [docs/internals.md — AI Gateway request headers](internals.md#ai-gateway-request-headers-user-agent-x-opencode-session)
